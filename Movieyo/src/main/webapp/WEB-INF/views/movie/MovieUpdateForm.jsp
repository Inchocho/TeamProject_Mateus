		...
		<div style="display: flex;">
					...
					<c:choose>
						<c:when test="${empty fileList}">
							<div>
								...
								<div class="file">
									<label for="file">사진등록</label> <a href="#this"
										id="delete_${obj.index}" class="deleteC">사진삭제</a><br> <input
						...
						<c:otherwise>
							<c:forEach var="row" items="${fileList}" varStatus="obj">
								...
								<img alt="image not found"
									src='<c:url value="/image/${row.STORED_FILE_NAME}"/>'>
								<br>
								<%-- 							${row.ORIGINAL_FILE_NAME}  --%>
								<%-- 							<input type="file" id="file_${obj.index}" name="file_${obj.index}"> --%>
								<%-- 							(${row.FILE_SIZE}kb) --%>
								<div class="file">
									<label for="file">사진변경</label> <a href="#this"
										id="delete_${obj.index}" class="deleteC">사진삭제</a><br> <input
										type='file' name='file' id="file">
								</div>
							</c:forEach>
			...
			<div style="">
				...
				영화제목: <input type='text' name='movieTitle'
					value='${movieDto.movieTitle}'><br> 제작연도 :<input
					type="text"
					value="<fmt:formatDate pattern="yyyy"
								value="${movieDto.prdtYear}"/>"
					name="prdtYear"><br> 국가 : <input type="text"
					name="nation" value="${movieDto.nation}"> <br> 감독 : <input
					type="text" name="director" value="${movieDto.director}"> <br>
				장르 :
				...
				</select> <br> 장르명 : <input type="text" name="genreName"
					value="${movieDto.genreName}" id="genreName"><br> 상영시간
				: <input type="text" name="runtime" value="${movieDto.runtime}">
				<br> 연령등급 : <input type="text" name="grade"
					value="${movieDto.grade}"> <br> 가격 : <input
					type="number" name="price" step="1000" value="${movieDto.price}">
				<br> 등록일 :
				<fmt:formatDate pattern="yyyy-MM-dd" value="${movieDto.creDate}" />
			</div>
		</div>
		<div>
			<br> 영화내용 <br>
			<textarea name="movieStory" style="width: 800px; height: 400px;">${movieDto.movieStory}</textarea>
			<br> <input type='submit' value='저장하기'> <input
